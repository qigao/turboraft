                                 ...
                                 uint32_t *out_ticks)
{
    ...
    *out_ticks = node->core_config.initial_election_timeout_ticks;
    return *out_ticks == 0u ? SALTS_EINVAL : SALTS_OK;
}
                                 ...
                                 const tr_text_replay_action_t *action)
{
    ...
    return status.role == expected ? SALTS_OK : SALTS_EPROTO;
}
                                   ...
                                   const tr_text_replay_action_t *action)
{
    ...
    return status.commit_index >= action->value ? SALTS_OK : SALTS_EPROTO;
}
