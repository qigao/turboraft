...
spec("text replay core driver") {
  it("elects a leader and applies a submitted command") {
    ...
    check_equal(tr_replay_driver_test_create(&driver, &state), SALTS_OK);
    check_equal(tr_replay_driver_test_node_action(driver, 1u), SALTS_OK);
    check_equal(tr_replay_driver_test_node_action(driver, 2u), SALTS_OK);
    check_equal(tr_replay_driver_test_node_action(driver, 3u), SALTS_OK);
    check_equal(tr_replay_driver_test_node_action(driver, 9u),
                 SALTS_ENOENT);

    check_equal(tr_replay_driver_test_tick(driver, 30u), SALTS_OK);
    ...
    check_equal(tr_replay_driver_step(driver, &action), SALTS_OK);
    ...
    check_equal(tr_replay_driver_test_tick(driver, 5u), SALTS_OK);
    for (index = 1u; index <= TEST_NODE_COUNT; ++index) {
  ...
  it("steps down without quorum and recovers after heal") {
    ...
    check_equal(tr_replay_driver_test_tick(driver, 12u), SALTS_OK);
    {
        tr_raft_status_t status;
  ...
  it("rejects unknown message kinds, bad targets, and duplicate requests") {
    ...
    check_equal(tr_replay_driver_step(driver, &action), SALTS_EINVAL);
    ...
    check_equal(tr_replay_driver_step(driver, &action), SALTS_ENOENT);
    ...
    check_equal(tr_replay_driver_test_tick(driver, 30u), SALTS_OK);
    {
        ...
        check_equal(tr_replay_driver_step(driver, &action), SALTS_OK);
        check_equal(tr_replay_driver_step(driver, &action), SALTS_EALREADY);
    }
    tr_replay_driver_destroy(driver);
  }
  ...
  it("rejects expectation failures and invalid ticks") {
    ...
    check_equal(tr_replay_driver_step(driver, &action), SALTS_EPROTO);
    ...
    action.kind = TR_TEXT_REPLAY_TICK;
    action.value = 0u;
    check_equal(tr_replay_driver_step(driver, &action), SALTS_ERANGE);
  ...
  it("runs a full parsed replay plan including send") {
    ...
    check_equal(tr_text_replay_parse(plan_text, strlen(plan_text), NULL,
                                      &plan, &diagnostic),
                 SALTS_OK);
    check_equal(tr_replay_driver_run(driver, &plan), SALTS_OK);
    check_equal(tr_replay_driver_status(driver, 2u, &status), SALTS_OK);
