...
spec("native raft replication")
{
    ...
    it("fills a configured peer window within Ready capacity")
    {
        ...
        check_int_eq(tr_raft_core_step(core, &response, &ready), TURBO_OK);
        check_size_eq(ready.message_count, 2U);
        check_long_eq(ready.messages[0].previous_log_index, 1U);
        check_long_eq(ready.messages[0].entries[7].index, 9U);
        check_long_eq(ready.messages[1].previous_log_index, 9U);
        check_long_eq(ready.messages[1].entries[7].index, 17U);
        check_int_eq(tr_raft_core_advance(core), TURBO_OK);

        ready = replication_ready_capacity(messages, 8U);
        check_int_eq(tr_raft_core_poll(core, &ready), TURBO_OK);
        check_size_eq(ready.message_count, 2U);
        check_long_eq(ready.messages[0].previous_log_index, 17U);
        check_long_eq(ready.messages[0].entries[7].index, 25U);
        check_long_eq(ready.messages[1].previous_log_index, 25U);
        check_long_eq(ready.messages[1].entries[7].index, 33U);
        check_int_eq(tr_raft_core_progress(core, &progress), TURBO_OK);
        check_size_eq(progress.peers[1].inflight_append_count, 4U);
    ...
    it("releases cumulative acknowledgements and probes after rejection")
    {
        ...
        ready = replication_ready_capacity(messages, 8U);
        check_int_eq(tr_raft_core_step(core, &response, &ready), TURBO_OK);
        check_size_eq(ready.message_count, 4U);
        check_int_eq(tr_raft_core_advance(core), TURBO_OK);
        ...
        check_int_eq(tr_raft_core_step(core, &response, &ready), TURBO_OK);
        check_size_eq(ready.message_count, 1U);
        check_long_eq(ready.messages[0].previous_log_index, 33U);
        check_long_eq(ready.messages[0].entries[0].index, 34U);
        check_int_eq(tr_raft_core_progress(core, &progress), TURBO_OK);
        check_long_eq(progress.peers[1].match_index, 17U);
        check_size_eq(progress.peers[1].inflight_append_count, 3U);
        check_int_eq(tr_raft_core_advance(core), TURBO_OK);
        ...
        check_size_eq(ready.messages[0].entry_count,
                      TR_RAFT_MAX_APPEND_ENTRIES);
        check_int_eq(tr_raft_core_progress(core, &progress), TURBO_OK);
        check_size_eq(progress.peers[1].inflight_append_count, 1U);
        check_true(progress.peers[1].append_probe);
        check_long_eq(progress.peers[1].next_index, 26U);
        check_int_eq(tr_raft_core_advance(core), TURBO_OK);
    ...
    it("resets a timed-out pipeline to one probe from confirmed progress")
    {
        ...
        ready = replication_ready_capacity(messages, 8U);
        check_int_eq(tr_raft_core_step(core, &response, &ready), TURBO_OK);
        check_size_eq(ready.message_count, 3U);
        check_int_eq(tr_raft_core_advance(core), TURBO_OK);
        ...
        check_size_eq(progress.peers[1].inflight_append_count, 1U);
        check_true(progress.peers[1].append_probe);
        check_long_eq(progress.peers[1].match_index, 1U);
        check_long_eq(progress.peers[1].next_index, 10U);
        tr_raft_core_destroy(core);
    }
}
