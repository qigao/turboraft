...
spec("raft membership transition")
{
    it("stages and applies a joint configuration at its commit boundary")
    {
        ...
        check_int_eq(tr_raft_membership_transition_stage(
                     ...
                     TURBO_OK);
        check_long_eq(tr_raft_membership_transition_commit_limit(
                          &transition, 12U),
                      10U);
        check_int_eq(tr_raft_membership_transition_apply(&transition, 9U),
                     TURBO_OK);
        check_int_eq(
            tr_raft_membership_transition_committed(&transition)->phase,
            TR_RAFT_CONF_FINAL);
        check_int_eq(tr_raft_membership_transition_apply(&transition, 10U),
                     TURBO_OK);
        check_int_eq(
            tr_raft_membership_transition_committed(&transition)->phase,
            TR_RAFT_CONF_JOINT);
    ...
    it("accepts a follower batch containing matching joint and final entries")
    {
        ...
        check_int_eq(tr_raft_membership_transition_stage(
                     ...
                     TURBO_OK);
        check_size_eq(transition.pending_count, 2U);
        check_int_eq(tr_raft_membership_transition_stage(
                         &transition, &final_membership, 22U),
                     TURBO_EPROTO);
        check_size_eq(transition.pending_count, 2U);
        check_int_eq(tr_raft_membership_transition_apply(&transition, 21U),
                     TURBO_OK);
        check_size_eq(transition.pending_count, 0U);
        check_int_eq(
            tr_raft_membership_transition_committed(&transition)->phase,
            TR_RAFT_CONF_FINAL);
        check_int_eq(tr_raft_membership_index(
                         tr_raft_membership_transition_committed(&transition),
                         3U),
                     -1);
        check_int_eq(tr_raft_membership_roles(
                         tr_raft_membership_transition_committed(&transition),
                         2U),
