...
spec("raft leadership transfer")
{
    it("sends TimeoutNow immediately to a caught-up voter and blocks proposals")
    {
        ...
        transfer_elect(core);
        ready = transfer_ready(messages);
        check_int_eq(tr_raft_core_transfer_leadership(core, 2U, &ready),
                     TURBO_OK);
        check_size_eq(ready.message_count, 1U);
        check_int_eq(ready.messages[0].type, TR_RAFT_MSG_TIMEOUT_NOW);
        ...
        check_long_eq(status.leadership_transfer_target, 2U);
    ...
    it("cancels a stalled transfer after one election timeout")
    {
        ...
        ready = transfer_ready(messages);
        check_int_eq(tr_raft_core_propose(core, &first, &ready), TURBO_OK);
        check_int_eq(tr_raft_core_advance(core), TURBO_OK);
        ready = transfer_ready(messages);
        check_int_eq(tr_raft_core_transfer_leadership(core, 2U, &ready),
                     TURBO_OK);
        ...
        check_long_eq(status.leadership_transfer_target, 0U);

        ready = transfer_ready(messages);
        check_int_eq(tr_raft_core_propose(core, &second, &ready), TURBO_OK);
        tr_raft_core_destroy(core);
    }
}
